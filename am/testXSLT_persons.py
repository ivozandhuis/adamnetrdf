...
# initiate XSLT
xslt = etree.parse("adlibPersonXML2rdf.xslt")
...
while (numberFound > (page * numberShow)):
    ...
    requestUrl = "http://amdata.adlibsoft.com/wwwopac.ashx?database=AMPerson&search=all&limit=" + str(numberShow) + "&startfrom=" + str(start)
    print(requestUrl)
...
#    numberFound = 100 # by overwriting you can shortcut for testing
    ...
    # write rdfxml-file
#    filename = "amPersons" + str(page) + ".rdf.xml"
#    f = open(filename,"wb")
#    f.write(rdfxml)
#    f.close()
    ...
    # write turtle-file
    filename = "amPersons" + str(page) + ".ttl"
    f = open(filename,"wb")
    f.write(s)
    f.close()
